...
const WHATSAPP_NUMBER = "923000000000"; // Replace with your actual WhatsApp number e.g. 923001234567
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-[#0f0f0f] border-l border-border/40 z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-border/40">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-serif text-lg tracking-wide">Your Collection</h2>
            {itemCount > 0 && (
              <span className="text-xs rounded-full px-2 py-0.5 font-medium text-black"
        ...
        <div className="flex-1 overflow-y-auto py-4 px-6 space-y-5">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-20">
              ...
              <p className="text-muted-foreground font-light text-sm">Your collection is empty.</p>
              <button onClick={closeCart} className="text-xs uppercase tracking-widest text-primary border-b border-primary pb-0.5 hover:opacity-80 transition-opacity">
                Explore Collections
              </button>
            </div>
          ) : (
            items.map((item) => (
              <div key={item.id} className="flex gap-4 pb-5 border-b border-border/30 last:border-0">
                <div className="w-20 h-24 bg-muted overflow-hidden shrink-0">
                  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-serif text-sm text-foreground leading-snug mb-1 truncate">{item.name}</h3>
                  <p className="text-primary text-sm font-light mb-3">${item.price.toFixed(2)}</p>
                  {/* Quantity controls */}
                ...
                <div className="text-right shrink-0">
                  <span className="text-sm font-light tracking-wide">${(item.price * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            ))
          )}
        ...
        {items.length > 0 && (
          <div className="border-t border-border/40 px-6 py-6 space-y-5 bg-[#0c0c0c]">
            {/* Rainbow divider */}
            <div className="w-full h-[1px] mb-2" style={{ background: "linear-gradient(90deg, #d4af37, #e8729a, #a78bfa, #60a5fa, #34d399)" }} />

            <div className="flex items-center justify-between">
              <span className="text-sm uppercase tracking-widest text-muted-foreground">Order Total</span>
              <span className="font-serif text-2xl text-foreground">${total.toFixed(2)}</span>
            </div>

            <p className="text-xs text-muted-foreground font-light leading-relaxed">
              Shipping & taxes calculated at checkout. Our team will confirm your order via WhatsApp within 24 hours.
            </p>
